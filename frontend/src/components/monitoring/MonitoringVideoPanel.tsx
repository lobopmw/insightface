...
export function MonitoringVideoPanel({ cameraStatus, isActive, websocketStatus }: MonitoringVideoPanelProps) {
  ...
  return (
    <section className="relative aspect-video min-h-[320px] overflow-hidden rounded-lg border border-slate-800 bg-slate-950 text-white shadow-sm">
      ...
      {isActive ? (
        ...
        <div className="flex h-full w-full items-center justify-center bg-[radial-gradient(circle_at_50%_30%,rgba(59,130,246,0.20),transparent_34%),linear-gradient(135deg,rgba(15,23,42,0.20),rgba(15,23,42,0.92))]">
          <div className="text-center">
            ...
            <p className="mt-4 text-sm text-white/75">Area principal para video da camera</p>
            <p className="mt-1 text-xs text-white/50">Inicie uma sessao para abrir o MJPEG do backend</p>
          </div>
        </div>
      )}
    </section>
